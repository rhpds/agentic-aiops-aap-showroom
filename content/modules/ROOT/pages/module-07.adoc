...
[NOTE]
====
The `--break-first` flag first runs the *Break Apache* template to simulate a real incident, then lets the agents respond -- reasoning autonomously, but executing all actions through Ansible Automation Platform.
====
...
* AI agents handle the *non-deterministic* work autonomously -- reasoning, triaging, and deciding what to do
* Ansible Automation Platform handles the *deterministic* work -- executing tested playbooks, enforcing access controls, recording audit trails
...
* Ansible Automation Platform provides the *governance layer*: user-scoped identity, RBAC, audit trails, and credential management
* This is the *autonomous* model: AI reasons and decides, Ansible Automation Platform executes deterministically, human reviews after
